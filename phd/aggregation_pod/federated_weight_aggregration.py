...
MASTER_WEIGHTS_PATH_RECEIVE = "/app/master_received_weights_json"
FEDERATED_WEIGHTS_PATH_SEND = "/app/federated_send_results"
...
def aggregate_weights_json(json_file_paths,target_resource):
    ...
    print(f"{blue_color_code}[INFO] {reset_color_code} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} \n MESSAGE:Aggregated weights saved to {AGGREGATED_JSON_FILES}/{target_resource}_weights_aggregated.json", flush=True)
...
while True:
    while True:
        ...
        while len(os.listdir(MASTER_WEIGHTS_PATH_RECEIVE))<=1:
            print(" ")
            print(f"{red_color_code}[INFO]:{reset_color_code} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} \n MESSAGE: {len(os.listdir(MASTER_WEIGHTS_PATH_RECEIVE))} nodes weights have been loaded, need for more", flush=True)
            time.sleep(5)
        break
